...
The search index is generated by `layouts/page/search.json` and is fetched only
after a query is entered. It includes plain article text so results can match
the body, in addition to title, summary, description, and metadata. This is an
intentional tradeoff for recall; check the per-language JSON size against the
`searchJsonBytes` budget after changing article content or ranking fields.
